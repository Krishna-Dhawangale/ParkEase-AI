...
// 1. Read temp_dt.tsx to get the canvas block
const tempDtContent = fs.readFileSync('d:/ParkEase AI/temp_dt.tsx', 'utf8');

const boardStart = tempDtContent.indexOf('<div\n              ref={boardRef}');
// Find the exact end of the board
...
// We want to slice up to the end of the `</div>\n            </div>`
// Since boardEndMatch points to the start of `              </div>\n            </div>\n  \n            <div className="rounded-lg border`,
// we will slice right up to boardEndMatch. But wait, `refactor3.cjs` wanted to slice up to `</div>\n          </div>\n\n          <div className="rounded-lg border` which was 2 divs closing.
// So slicing up to `boardEndMatch` gives us the exact content!
let canvasContent = tempDtContent.substring(boardStart, boardEndMatch);

// Now apply our specific fixes to canvasContent!
// Fix pan logic
...
// Apply clip-path and border-radius to board!
canvasContent = canvasContent.replace(
...
// Apply box-shadow rendering fix
canvasContent = canvasContent.replace(
...
if (returnIndex !== -1) {
  ...
  // Make sure we have our missing functions: updateCanvasSettings, saveLayout fix, shape injects!
  let fixedHead = newLines.join('\n');
  
  // Replace the old drop logic to include shape
  ...
  // If the previous fix already replaced it, don't double replace
...
`;
  ...
  // Remove BuilderSnapshot locally if it exists
  ...
  fixedHead = fixedHead.replace(/type BuilderSnapshot = \{[\s\S]*?\};\r?\n/, '');
