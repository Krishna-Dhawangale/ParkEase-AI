...
function StepSeven() {
  ...
  const slotName = searchParams.get('slot') || 'A-12';
  const floorName = searchParams.get('floor') || 'Ground Floor';
  
  // Confirm the booking in store on mount
  useState(() => {
    confirmBooking();
  });
  return (
...
export function Checkout() {
  const [searchParams] = useSearchParams();
  const initialStep = searchParams.has('slot') ? 5 : 1;
