      ...
      <form id="new-facility-form" onSubmit={handleSubmit} className="max-w-2xl mx-auto px-4 md:px-6 pt-8 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Latitude" hint="For map integration (optional now)">
                <input type="text" value={latitude} onChange={e => setLatitude(e.target.value)} placeholder="19.0760" className={inputCls} />
              </Field>
              <Field label="Longitude">
                <input type="text" value={longitude} onChange={e => setLongitude(e.target.value)} placeholder="72.8777" className={inputCls} />
              </Field>
            </div>
            <p className="text-xs text-slate-400 flex items-center gap-1.5">
              <MapPin className="w-3 h-3" /> Google Maps integration will be available in the next release.
            </p>
          </div>
        </div>
