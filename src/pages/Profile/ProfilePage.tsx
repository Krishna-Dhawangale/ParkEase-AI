    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
          ...
          {/* Stats */}
          <div className="grid grid-cols-3 gap-4 sm:border-l sm:border-[#E5E7EB] sm:dark:border-[#334155] sm:pl-6">
            {[
              { icon: Car, label: 'Total Trips', value: '142', color: 'text-[#0F766E]' },
          ...
          {[
            ...
            >
              <div className={cn(
                ...
                item.color === 'brand' && 'bg-[#0F766E]/10 dark:bg-[#14B8A6]/10',
                ...
                <item.icon className={cn('w-4.5 h-4.5', {
                  'text-[#0F766E] dark:text-[#14B8A6]': item.color === 'brand',
              ...
              <div className="text-lg font-bold text-[#111827] dark:text-white">{item.value}</div>
              <div className="text-xs font-medium text-[#6B7280]">{item.label}</div>
              <div className="text-[11px] text-[#9CA3AF] mt-0.5">{item.sub}</div>
            </motion.div>
          ))}
        </div>
      </ProfileTab>
      ...
      <ProfileTab active={activeTab === 'Vehicles'}>
        <div className="space-y-3">
          {[
            ...
            >
              <div className="w-12 h-12 rounded-2xl bg-[#F8FAFC] dark:bg-[#334155] flex items-center justify-center">
                <Car className="w-6 h-6 text-[#6B7280]" />
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-[#111827] dark:text-white">{vehicle.number}</span>
                  {vehicle.primary && <span className="badge badge-brand text-[10px]">Primary</span>}
                </div>
                <div className="text-sm text-[#6B7280] dark:text-[#94A3B8]">{vehicle.brand} · {vehicle.type} · {vehicle.color}</div>
              </div>
          ...
          <button className="card w-full p-4 border-dashed flex items-center justify-center gap-2 text-sm font-semibold text-[#0F766E] dark:text-[#14B8A6] hover:bg-[#0F766E]/5 transition-colors">
      ...
      <ProfileTab active={activeTab === 'History'}>
        <div className="card overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E5E7EB] dark:border-[#334155]">
            <h3 className="font-bold text-[#111827] dark:text-white">Booking History</h3>
          </div>
          <div className="divide-y divide-[#E5E7EB] dark:divide-[#334155]">
            {mockRecentBookings.map((booking, i) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.08 }}
                className="flex items-center gap-4 px-5 py-4 hover:bg-[#F8FAFC] dark:hover:bg-[#334155]/30 transition-colors cursor-pointer"
              >
                <div className="w-9 h-9 rounded-xl bg-[#0F766E]/10 dark:bg-[#14B8A6]/10 flex items-center justify-center">
                  <Car className="w-4.5 h-4.5 text-[#0F766E] dark:text-[#14B8A6]" size={18} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[#111827] dark:text-white truncate">{booking.parking}</p>
                  <p className="text-xs text-[#6B7280]">Slot {booking.slot} · {booking.time} · {booking.duration}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-sm font-bold text-[#111827] dark:text-white">₹{booking.amount}</p>
                  <span className={cn(
                    'text-[10px] font-bold px-2 py-0.5 rounded-full',
                    booking.status === 'active' && 'bg-[#0F766E]/10 text-[#0F766E]',
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={r.title} className="card p-4">
                ...
                <div className="font-bold text-[#111827] dark:text-white">{r.value}</div>
                <div className="text-xs text-[#6B7280] font-medium">{r.title}</div>
                <div className="text-[11px] text-[#9CA3AF]">{r.desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={section.title} className="card overflow-hidden">
              <div className="px-5 py-3 border-b border-[#E5E7EB] dark:border-[#334155]">
                <p className="text-xs font-bold text-[#9CA3AF] uppercase tracking-wider">{section.title}</p>
              </div>
              <div className="divide-y divide-[#E5E7EB] dark:divide-[#334155]">
                {section.items.map(item => (
                  <div key={item.label} className="flex items-center justify-between px-5 py-4 hover:bg-[#F8FAFC] dark:hover:bg-[#334155]/30 transition-colors cursor-pointer"
                    onClick={item.label === 'Dark Mode' ? toggleTheme : undefined}
                  >
                    <div className="flex items-center gap-3">
                      <item.icon className="w-4 h-4 text-[#6B7280]" />
                      <span className="text-sm text-[#111827] dark:text-white">{item.label}</span>
                    </div>
                    {item.action === 'toggle' ? (
                      <div className={cn(
                        'w-10 h-6 rounded-full relative transition-colors cursor-pointer',
                        item.value ? 'bg-[#0F766E]' : 'bg-[#E5E7EB] dark:bg-[#334155]'
                      )}>
                        <div className={cn(
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-[#6B7280]">{item.value}</span>
