...
const minimapEnd = tempDtContent.indexOf('background: item.stroke }}');
if (boardStart === -1 || minimapEnd === -1) {
  console.error("Could not find start or minimapEnd in temp_dt.tsx");
  process.exit(1);
}

// Find the 4 closing divs after minimapEnd
let currentIdx = minimapEnd;
for (let i = 0; i < 4; i++) {
  currentIdx = tempDtContent.indexOf('</div>', currentIdx + 1);
}
// Add the length of '</div>'
const boardEndMatch = currentIdx + 6;
...
`;
  ...
  console.log('Successfully recovered DigitalTwin.tsx completely!');
} else {
