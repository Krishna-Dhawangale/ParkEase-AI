    ...
    <div className="space-y-6 max-w-[1600px] mx-auto text-txt-primary">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-bg-card p-6 rounded-2xl border border-bdr shadow-xl">
        <div>
          ...
          <h1 className="text-2xl font-bold text-txt-primary tracking-tight">Facility Gate Barriers & ANPR Logs</h1>
          <p className="text-xs text-txt-secondary mt-1">Real-time Automatic Number Plate Recognition (ANPR) logs, barrier controls, security incidents, and blacklist manager.</p>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-bg-card border border-bdr space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-txt-primary text-base">ANPR Camera Live Scan Logs</h3>
          </div>
            ...
            <tbody className="divide-y divide-bdr">
              {mockAnprLogs.map(log => (
                <tr key={log.id} className="hover:bg-bg-elevated/40 transition-colors">
                  <td className="p-3 font-mono font-bold text-txt-secondary">{log.id}</td>
                  <td className="p-3 font-mono text-base font-bold text-txt-primary">{log.plate}</td>
                  <td className="p-3 text-txt-secondary">{log.gate}</td>
                  <td className="p-3 text-txt-secondary">{log.timestamp}</td>
                  <td className="p-3 text-emerald-400 font-semibold">{log.confidence}%</td>
                  <td className="p-3 font-medium text-txt-secondary">{log.status}</td>
                  <td className="p-3">
