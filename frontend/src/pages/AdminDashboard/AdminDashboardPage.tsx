    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-[1600px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl gradient-brand flex items-center justify-center">
            <Shield className="w-5 h-5 text-txt-primary" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-txt-primary tracking-tight">
              Admin Command Center
            </h1>
            <p className="text-sm text-txt-secondary flex items-center gap-1.5">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-txt-primary">System Health:</span>
          <span className="text-lg font-bold text-green-600">{stats.healthScore}/100</span>
        </div>
        <div className="h-2 flex-1 min-w-32 bg-[var(--border)] dark:bg-bg-secondary rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
        ...
        <div className="flex items-center gap-3 text-xs">
          {[
            ...
            <div key={item.label} className="flex items-center gap-1">
              ...
              <span className="text-txt-secondary">{item.label}: <span className={cn('font-semibold', item.ok ? 'text-green-600' : 'text-red-600')}>{item.value}</span></span>
            </div>
          ))}
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          const colorMap: Record<string, string> = {
            brand: 'bg-brand/10 text-brand dark:bg-[var(--brand-light)]/10 dark:text-[var(--brand-light)]',
          ...
          return (
            ...
            >
              ...
              <div className="text-xl font-bold text-txt-primary">{kpi.value}</div>
              <div className="text-[11px] text-txt-secondary">{kpi.label}</div>
            </motion.div>
          );
        })}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <motion.div
          ...
          className="card p-5"
        >
          <h3 className="font-bold text-txt-primary mb-4">Slot Distribution</h3>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-txt-primary">Live Occupancy & Revenue</h3>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-txt-primary">Floor Occupancy</h3>
          </div>
              ...
              return (
                <div key={f.floor}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-semibold text-txt-primary">{f.floor}</span>
                    <span className="text-xs text-txt-secondary">{f.occupancy}/{f.total} ({pct}%)</span>
                  </div>
                  <div className="h-3 bg-[var(--border)] dark:bg-bg-secondary rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${pct}%` }}
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-bdr">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-txt-primary">Alerts</h3>
            </div>
          ...
          <div className="divide-y divide-bdr">
            {alerts.map((alert, i) => (
              <motion.div
                ...
                className="px-5 py-3 hover:bg-bg-primary dark:hover:bg-[var(--border)]/30 transition-colors cursor-pointer"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-txt-primary">{alert.title}</p>
                    <p className="text-[11px] text-txt-secondary truncate">{alert.desc}</p>
                  </div>
                  <span className="text-[10px] text-txt-secondary whitespace-nowrap">{alert.time}</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-bdr">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-txt-primary">Staff Activity</h3>
            </div>
            <span className="text-xs text-txt-secondary">3 online</span>
          </div>
            ...
            {employeeActivity.map((emp, i) => (
              ...
              >
                <div className="relative">
                  <div className="w-8 h-8 rounded-full bg-brand/10 dark:bg-[var(--brand-light)]/10 flex items-center justify-center text-xs font-bold text-brand">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-txt-primary">{emp.name} · <span className="text-txt-secondary font-normal">{emp.role}</span></p>
                  <p className="text-[11px] text-txt-secondary truncate">{emp.action}</p>
                </div>
                <span className="text-[10px] text-txt-secondary whitespace-nowrap">{emp.time}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
      ...
      <motion.div
        ...
        className="card p-5"
      >
        <h3 className="font-bold text-txt-primary mb-4">Security & Access Control</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={gate.label} className={cn(
              ...
              <div className="text-xs font-semibold text-txt-primary">{gate.label}</div>
              <div className={cn(
